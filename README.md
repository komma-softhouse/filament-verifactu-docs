...
The two binaries (`pos-agent.exe`, `nssm.exe`) are not part of the
Composer package: the installer downloads them from
`VERIFACTU_AGENT_BINARIES_URL` — by default the host's own
`public/downloads/windows/marfil-printer-agent/`, where you place the copies
you distribute; set it to an absolute URL to serve them from a CDN or object
storage instead. `/download/windows/{file}` redirects there for installers
generated before 1.3.5.
...
full terms.  action carries the legal context), respecting the AEAT's flow-control
  window. The continuous pipeline refuses non-Verifactu issuers by design:
  their records only ever leave on demand.
- **Reads back from the AEAT** (`ConsultaFactuSistemaFacturacion`): asks
  the office what IT holds for a record — state, presentation data, and
